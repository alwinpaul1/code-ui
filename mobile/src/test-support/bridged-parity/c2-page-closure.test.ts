...
describe('the C2 page closure', () => {
   ...
   */
  // Code UI: 68 families / 257 goldens over this fork's corpus (upstream's census is 70 / 266; the
  // two families short are named in the pin's own docstring).
  ...
  it('pins how many goldens land in each class, which a per-id walk cannot see move', () => {
    // Code UI: this fork's totals; upstream's are 114 / 118 / 29 / 3 / 2 over 266.
  ...
  it('agrees with C5 object for object on every family both domains pin', () => {
    ...
    // Code UI: C1's 20 here, where upstream's line reads 22.
  ...
  it('names the families where a pin proves only that the divergence kept its name', () => {
    ...
    // Code UI: sorted by name, since this fork's tables are emitted largest family first rather
    // than in upstream's hand order; the six families and their counts are upstream's own.
  ...
  it('excludes a closure golden only into a class that has a reason', () => {
    ...
    // Code UI: 149 over this fork's corpus, where upstream's line reads 152.
...
describe('reading a run against the C2 pin', () => {
  ...
  it('names a closure golden the run stopped producing, three ways', () => {
    ...
    // Code UI: 256 (257 − 1) over this fork's corpus, where upstream's line reads 265.
