 ...
 * bridge.
 *
 * CODE UI: the numbers in this docstring are upstream's (70 families / 266 goldens over a
 * 787-golden corpus). This fork's closure of `app/h/_layout.tsx` and `app/h/[hostId]/tasks.web.tsx`
 * is 423 of its own modules and 68 families / 257 goldens over its 759-golden corpus: C1's 20
 * inherited (upstream's 22 minus `notifications.push-registration`, which this fork has no feature
 * for, and `transport.capability-probe`, whose probe is not a recorded operation here yet) plus the
 * same 26 work-item and 22 task-source families upstream adds. The six wholly divergent families
 * are the same six.
 *
