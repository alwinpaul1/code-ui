 ...
 *
 * CODE UI (2026-09-23, Orca #21777 port): the corpus and module counts above are upstream's. Over
 * this fork's 759-golden corpus the bridged run reads the same table exactly: 4 families, 15
 * goldens, 6 identical and 9 `result-absent-settlement`, all recorded at the same two sites
 * (`browser-mount-adapters.ts` mounts nothing else under `src/browser`). The module-closure census
 * that proves "the pane reaches exactly these four" is upstream's `config/scripts`, which this fork
 * does not vendor, so the site list is the check here.
 */
