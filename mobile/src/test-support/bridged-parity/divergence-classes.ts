...
/**
 * What this tree measures, per class, over all 264 goldens.
 *
 ...
 *
 * CODE UI (2026-09-19, Orca #21485 port): re-pinned from a measured run over this fork's own
 * corpus — 264 goldens, not upstream's 787 — the way this file's own rule says. `write-ordinal` is
 * 0 here because this recorder has no subscription lane yet (Orca #20920/#21088, Group D), so
 * nothing publishes a payload a delivery later than the call that made it.
 */
export const BRIDGED_PARITY_BASELINE: Readonly<Record<BridgedParityClass | 'identical', number>> = {
  identical: 13,
  'reply-meta-required': 133,
