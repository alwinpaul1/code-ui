...
describe('a scenario that declares its device', () => {
  ...
  // CODE UI: upstream also proves the declared tray end to end through its
  // `push-dismissal-tray-reconciled` recording (push-dismissal-reconciliation.ts). This fork has no
  // such module — its push delivery is its own — so no scenario declares a tray yet and that case
  // is not carried; the unit case above is what pins the capability here.
})
