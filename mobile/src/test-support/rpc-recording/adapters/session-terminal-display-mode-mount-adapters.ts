      ...
      const screen = hookScreenMount(() => {
        display = useStreamDisplay(
          mountFixture<Parameters<typeof useStreamDisplay>[0]>({
            ...
            // CODE UI: this fork measures a never-shown terminal before asking for 'auto' (a tab
            // that opened as chat has no phone dims to drive at). A device measurement is not an
            // observation this oracle holds, so it is recorded as the choice and measures nothing;
            // the viewport cell stays whatever the scenario declared.
            measureViewportOnce: (handle: string) => {
              effect('measure-viewport-once', { handle })
