...
export function sessionStartupMountAdapters(
  ...
  return {
    'session.startup': ({ client, effect }) => {
      ...
      // CODE UI: the in-flight ref counts sends per handle here (the wheel pipeline), not a set.
      const terminalGestureInputInFlightRef = { current: new Map<string, number>() }
      ...
      const hook = hookMount(() => {
        useStartup(
          mountFixture<Parameters<typeof useStartup>[0]>({
            ...
            setSessionTabs: () => {},
            // CODE UI: the startup seeds the strip from the last visit's cached tabs before the
            // host answers; with nothing cached these stay empty, which is what a first visit is.
