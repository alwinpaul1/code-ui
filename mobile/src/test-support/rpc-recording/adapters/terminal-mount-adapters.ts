...
/**
 * Terminal input and the in-place viewport refit. (Upstream also mounts the worker-takeover report
 * the raw send triggers there; CODE UI has no orchestration worker and its raw send reports
 * nothing, so that mount and its two scenarios are not carried.)
 *
