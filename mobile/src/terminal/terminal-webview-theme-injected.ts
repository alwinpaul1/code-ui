...
export const TERMINAL_WEBVIEW_THEME_JS = `
  ...
  var appliedTerminalThemeKey = null;
  ...
  function applyTerminalTheme(input) {
    ...
    // repaint. Skip identical payloads so tab switches do not churn the renderer.
    var themeKey;
    try {
      themeKey = JSON.stringify(input === undefined ? null : input);
    } catch (_error) {
      themeKey = null;
    }
