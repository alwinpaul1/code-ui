...
export function terminalDocumentDouble() {
  ...
  const terminal: TerminalDocumentTerminal = {
    ...
    // Code UI: the shape carries the parser because `query-reply` answers DECRQM through it
    // (c840bc19). Registering answers nothing here, as the engine would before a query arrives.
    parser: {
