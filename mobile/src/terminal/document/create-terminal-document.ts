...
import { stopOverscrollBend } from './overscroll-bend'
...
import { stopWriteQueue } from './write-queue'
...
export function stopTerminalDocument(scope: TerminalDocumentScope) {
  ...
  // Code UI: the scrollback's bend springs back on a frame (9a6507b6).
  stopOverscrollBend(scope)
  stopNormalBufferSmoothScroll(scope)
  stopHostNotify(scope)
  stopTerminalInit(scope)
  stopWebglRecovery(scope)
  // Code UI: the write hold's wake timer (8372594a), which would otherwise pump a stopped queue.
  stopWriteQueue(scope)
  stopFitScale(scope)
