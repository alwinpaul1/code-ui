...
// cannot drift apart.
// Code UI note: this fork's queue also carries the scroll-gesture write hold, whose state is on the
// scope and whose clock (`nowMs`, from viewport-transform) the harness stubs. The hold is inactive
// here, so the queue drains exactly as upstream's harness drives it.
