...
function requireDocumentModule(specifier: string) {
  ...
  // Code UI: this fork's queue also carries the scroll-gesture write hold, which reads the
  // high-resolution clock from viewport-transform. The hold is inactive in these cases, so a
  // clock that reads 0 lets the queue drain exactly as upstream's harness drives it.
  if (specifier === './viewport-transform') {
    return { nowMs: () => 0 }
  }
