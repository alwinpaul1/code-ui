...
describe('the document modules at parse time', () => {
  ...
  it('still start and stop: the functions holding what was moved out are exported', () => {
    ...
    // Code UI: 12, not upstream's 10 — this fork's write hold arms a wake timer
    // (`stopWriteQueue`) and its overscroll bend springs back on a frame (`stopOverscrollBend`).
    expect(declaring('stop').length).toBe(12)
  })
})
