...
describe('picking media from inside the shell', () => {
  ...
  it('refuses the camera, which the shell has no pick for, before anything is staged (CODE UI)', async () => {
    // This fork's composer offers Take photo beside the library and Files. The shell serves three
    // sources, so the camera rejects rather than answering the empty pick a cancel answers.
    const shell = libraryShell(1)
    const pair = pairFor(shell)
    const picker = await mount(pair)
    const before = pair.toShell.length

    const single = settle(pair, picker.pickImage('camera')).catch((error: unknown) => error)
    const multiple = collect(pair, picker.pickImages('camera')).catch((error: unknown) => error)

    expect(String(await single)).toMatch(/cannot open the camera/)
    expect(String(await multiple)).toMatch(/cannot open the camera/)
    expect(pair.toShell).toHaveLength(before)
    expect(shell.calls).toEqual([])
  })

  it('asks the shell for the pasteboard as its own source (CODE UI)', async () => {
    const shell = createMediaTestShell({ staged: {} })
    const pair = pairFor(shell)
    const picker = await mount(pair)

    expect(await collect(pair, picker.pickImages('clipboard'))).toEqual([])
    expect(shell.calls).toEqual(['pick clipboard multiple'])
  })
})
