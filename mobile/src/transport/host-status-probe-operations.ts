...
/**
 * CODE UI: `readProbedHostCapabilities` and the accepted status record it read them off, in one
 * interpretation. The capability probe hands the record to the session tab reconciliation, which
 * reads the host platform off it (a Windows host mutes the phone's terminal query replies);
 * upstream's callback carries only the capabilities. One read rather than two, because a checked
 * read that salvages is an observation the recorder counts, and reading the same reply twice
 * would record the salvage twice.
 */
export function readProbedHostStatus(
  reply: RpcResponse
): { capabilities: readonly string[]; status: unknown } | null {
  try {
    const accepted = hostStatusProbe.interpret(reply)
    return accepted.accepted
      ? { capabilities: accepted.value.capabilities ?? [], status: accepted.value }
      : null
  } catch {
    return { capabilities: [], status: null }
  }
}
