...
export function createMobileFilePreviewRequest(
  ...
  if (source.source === 'terminalArtifact') {
    const method = readsBinaryPreview(source.absolutePath)
      ? 'files.readTerminalArtifactPreview'
      : 'files.readTerminalArtifact'
    return {
      method,
  ...
  return {
    method: readsBinaryPreview(source.relativePath) ? 'files.readPreview' : 'files.read',
    params: {
      worktree: `id:${source.worktreeId}`,
...
/** Images and PDFs come back as base64 from the preview methods; everything else is text. */
function readsBinaryPreview(path: string): boolean {
...
export async function loadMobileFilePreview(
  ...
  // Why: the host's preview read is capped and refuses ordinary PDFs as
  // `file_too_large`; worktree PDFs page over in chunks instead. Terminal
  // artifacts keep the grant-scoped preview path.
  const worktreePdf =
    typeof source === 'string'
      ? { worktreeId: source, relativePath: relativePath! }
      : source.source === 'worktree'
        ? { worktreeId: source.worktreeId, relativePath: source.relativePath }
        : null
