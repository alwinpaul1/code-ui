...
describe('an action that did not happen', () => {
  it('is on screen above the list, in the tone that says it failed', () => {
    ...
    // Code UI: the banner paints from the live theme (the light fallback under test), not the
    // static palette upstream's asserts on, so the red rule is `colors.danger` on an inline style.
    const banner = tree.root.findAll(
      (node) =>
        String(node.type) === 'View' &&
        typeof node.props.style === 'object' &&
        node.props.style !== null &&
        'borderBottomColor' in node.props.style
    )[0]
    expect(banner?.props.style).toMatchObject({ borderBottomColor: lightColors.danger })
  })
