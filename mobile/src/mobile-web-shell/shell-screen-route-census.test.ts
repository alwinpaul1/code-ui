...
// CODE UI: upstream ties the root to the desktop's route registry (config/scripts/
// mobile-web-page-routes.mjs), which this fork does not vendor — the host sends the phone its page
// routes at runtime and route-handoff.web.ts matches them. The tie here is to the route files that
// mount the shell: their pathnames, read from where they sit under app/, must all be under the
// root this census walks.
...
/** Every route file under `app/` that mounts the shell, wherever it sits. */
function shellRouteFiles(dir: string): string[] {
  const found: string[] = []
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const path = join(dir, entry.name)
    if (entry.isDirectory()) {
      found.push(...shellRouteFiles(path))
    } else if (
      entry.name.endsWith('.tsx') &&
      !entry.name.includes('.web.') &&
      !entry.name.includes('.test.') &&
      readFileSync(path, 'utf8').includes('MobileWebShellScreen')
    ) {
      found.push(path)
    }
  }
...
describe('the switches that hand a route to the shell', () => {
  ...
  it('walks the directory every registered route lives under', () => {
    ...
    const pathnames = shellRouteFiles(APP_ROOT).map((file) => {
      const rel = file.slice(APP_ROOT.length).replace(/\.tsx$/, '')
