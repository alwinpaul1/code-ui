...
// CODE UI: upstream reads the desktop's route registry (config/scripts/mobile-web-page-routes.mjs),
// which this fork does not vendor; the host sends the phone its page routes at runtime. The
// coverage rule reads the switched route files instead — each `app/h/[hostId]/**` route that
// renders MobileWebShellScreen — which is the fork's own list, the one the flag census keeps.
const SWITCHED_ROUTE_ROOT = join(MOBILE_ROOT, 'app', 'h', '[hostId]')
...
function routeFiles(dir: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (entry.name.endsWith('.tsx') && !entry.name.includes('.web.') && !entry.name.includes('.test.')) {
      found.push(path)
    }
  }
  return found
...
/** The pathnames of the route files switched to the page shell, from their location under app/. */
function registeredPathnames(): string[] {
  return routeFiles(SWITCHED_ROUTE_ROOT)
    // `web.tsx` is the shell route itself, the page's host, not a screen it serves (the flag
    // census keeps it apart from the switched routes for the same reason).
    .filter((path) => !path.endsWith('/web.tsx') && readFileSync(path, 'utf8').includes('MobileWebShellScreen'))
    .map((path) => {
      const rel = path.slice(join(MOBILE_ROOT, 'app').length).replace(/\.tsx$/, '')
