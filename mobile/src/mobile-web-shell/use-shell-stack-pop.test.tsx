...
/** One shipped CommonJS file, evaluated against a stub `require`. */
function evaluateShipped(root: string, file: string, stubs: Record<string, unknown>): unknown {
  const source = readFileSync(join(root, 'build/global-state', file), 'utf8')
 ...
 *
 * Code UI: written against expo-router 57 (upstream's test loads 55's single `routing.js`). In 57
 * `routing.js` is a re-export shim; `routingQueue` lives in `routingQueue.js` and `canGoBack` /
 * `goBack` in `router.js`, which reads the navigation ref off `./store` rather than
 * `./router-store`. Both files are evaluated as shipped, with the queue handed to the router the
 * way the shim would, so the latch is still tested against the real queue and the real pop.
 */
function loadRoutingModule(ref: NavigationRef): RoutingModule {
  ...
  const queueModule = evaluateShipped(root, 'routingQueue.js', {})
  const stubs: Record<string, unknown> = {
    'expo/dom': { IS_DOM: false },
    './routingQueue': queueModule,
    './store': {
      store: { navigationRef: { isReady: () => true, ...ref }, assertIsReady: () => undefined }
    },
    '../domComponents/emitDomEvent': { emitDomGoBack: () => false }
  }
  const routerModule = evaluateShipped(root, 'router.js', stubs)
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the three members are read and checked for presence on the next lines; the shape is the module's own.
  const { canGoBack, goBack } = routerModule as Partial<RoutingModule>
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: same check below.
  const { routingQueue } = queueModule as Partial<RoutingModule>
