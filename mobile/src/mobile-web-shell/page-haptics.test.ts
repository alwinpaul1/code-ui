...
// CODE UI: this fork's vitest.setup.ts stubs `platform/haptics` for every test (the press
// primitives call it, and expo-haptics cannot load under a mocked react-native). This file is the
// one that must reach the real module, so it takes the stub back off.
vi.unmock('../platform/haptics')
