...
export type MacHostState = { lock: MacLockState; display: MacDisplayState }

export const UNKNOWN_MAC_HOST_STATE: MacHostState = { lock: 'unknown', display: 'unknown' }
...
export const MAC_HOST_STATE_PROBE_COMMAND =
  `printf '${MARKER} lock=%s display=%s\\n' ` +
...
const MARKER_PATTERN = new RegExp(`${MARKER} lock=([01]) display=(on|off)\\b`)
