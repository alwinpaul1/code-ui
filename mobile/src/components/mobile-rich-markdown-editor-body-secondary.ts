export const MOBILE_RICH_MARKDOWN_EDITOR_BODY_SECONDARY = [
  ...
  '          stack.push(item);',
  '          index += 1;',
  '          // A wrapped item continues on the lines under it: indented,',
  '          // non-blank, and not a marker of its own. Without this the list',
  '          // ENDED at the first continuation line and the rest of the item',
  '          // became a paragraph at the left margin, outside the list — which',
  '          // is what the device screenshot showed for numbered item 2.',
  '          var tail = [item.text];',
  '          while (index < lines.length) {',
  "            var next = String(lines[index] || '');",
  "            if (!next.trim() || !/^\\s/.test(next) || parseListLine(next) || isBlockStart(next)) {",
  '              break;',
  '            }',
  '            tail.push(next.trim());',
  '            index += 1;',
