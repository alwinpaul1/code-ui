...
describe('MobileHostCard', () => {
  ...
  it('keeps host navigation and actions as separate accessible controls', async () => {
    ...
    // The actions control keeps a 40x40 target. Its style is a spring-animated
    // array, so flatten before matching.
    expect(
      [buttons[1].props.style].flat(Infinity).filter((entry) => typeof entry === 'object')
    ).toEqual(expect.arrayContaining([expect.objectContaining({ width: 40, height: 40 })]))
    expect(renderer.root.findAllByType('MoreHorizontal')).toHaveLength(1)
    // The chevron is decoration on the card, not a third control.
  ...
  it('includes visible offline recovery guidance in the navigation label', async () => {
    ...
    const navigationButton = renderer.root.findAllByType('Pressable')[0]
    expect(navigationButton.props.accessibilityLabel).toBe(
      "Open Desk, Can't reach desktop, Update the desktop app and sign in to connect from anywhere"
    )
  })
})
