...
describe('buildSections', () => {
  ...
  it('shows pinned worktrees only under Pinned by default (#15494)', () => {
    const pinned = worktree({
      worktreeId: 'pinned',
      workspaceStatus: 'in-progress',
      isPinned: true
    })
    const unpinned = worktree({
      worktreeId: 'unpinned',
      workspaceStatus: 'in-progress',
      isPinned: false
    })
    ...
    expect(withoutSectionListKeys(sections)).toEqual([
      ...
      { key: 'workspace-status:in-progress', title: 'In progress', data: [unpinned] }
    ])
  })

  it('keeps pinned worktrees in their canonical group when desktop opts in', () => {
    const pinned = worktree({
      worktreeId: 'pinned',
      workspaceStatus: 'in-progress',
      isPinned: true
