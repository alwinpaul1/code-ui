...
export type MobileSessionViewModeController = {
  ...
  peekedTerminalTabId: string | null
  /** False until both the device default and this scope's overrides have been
   *  read. Until then `isTabChatView` is a placeholder answer, and chrome that
   *  differs between the two views should hold rather than flash terminal. */
...
export function useMobileSessionViewMode(args: {
  ...
  const peekedTerminalTabIdRef = useRef<string | null>(null)
  ...
  const peekTerminalTab = useCallback((tabId: string) => {
    setPeekedTerminalTabId(tabId)
  }, [])
  const endTerminalPeek = useCallback(() => {
    setPeekedTerminalTabId(null)
  }, [])
  ...
  peekedTerminalTabIdRef.current = peekedTerminalTabId
  const viewResolved =
    defaultViewLoaded && overridesSettled && isOverrideScope(viewOverridesState, hostId, worktreeId)
