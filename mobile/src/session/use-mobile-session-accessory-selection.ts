...
export function useMobileSessionAccessorySelection(scope: MobileSessionTerminalInputModel) {
  ...
  // Whether the current hold has auto-repeated at least once. Set by the
  // interval, read on release so a tap (no repeat yet) sends exactly once and a
  // hold (already repeating) sends nothing extra (#12251).
  const accessoryRepeatFiredRef = useRef(false)
  const didAccessoryRepeatFire = useCallback(() => accessoryRepeatFiredRef.current, [])
  ...
  const startAccessoryRepeat = useCallback(
    (input: ReturnType<typeof createTerminalLiveAccessoryInput>) => {
      stopAccessoryRepeat()
      accessoryRepeatFiredRef.current = false
      repeatTimeoutRef.current = setTimeout(() => {
        // Why (#12251): the first byte lands here, after the hold delay, not on
        // press-in. A swipe across the strip presses in and is cancelled by the
        // ScrollView before this fires, so scrolling no longer types.
        accessoryRepeatFiredRef.current = true
