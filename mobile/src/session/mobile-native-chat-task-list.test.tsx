...
describe('an agent plan in the chat transcript', () => {
  ...
  function render(blocks: NativeChatBlock[], scheme: 'light' | 'dark' = 'light') {
    act(() => {
      renderer = create(
        createElement(
          ThemeProvider,
    ...
    // Everything the tool line itself says: the one-line row preview and the
    // detail under it. Deliberately not the run header above them, which is
    // built by the vendored `toolRunSummaryMembers` and reads the same on
    // desktop — see the inventory's note on it.
    const line = renderer!.root
      .findAll((node) => node.props?.testID === 'tool-line-preview')
      .map((node) => String(node.props.children ?? ''))
    return { texts, tasks, line }
  }

  it("shows Claude's plan as a checklist instead of the raw JSON of its input", () => {
    const { texts, tasks, line } = render(CLAUDE_PLAN)
  ...
  it("shows Codex's plan too, under its own field names and its own spelling", () => {
    const { texts, tasks, line } = render(CODEX_PLAN)
    expect(tasks.map((task) => task.text)).toEqual([
      'Read the parser',
