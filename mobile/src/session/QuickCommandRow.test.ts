...
const haptics = vi.hoisted(() => ({ triggerError: vi.fn() }))
...
// CODE UI: vitest.setup.ts stubs '../platform/haptics' to no-ops for every test, so the buzz is
// read at the helper rather than at expo-haptics underneath it, as this fork's haptics tests do.
vi.mock('../platform/haptics', () => ({ triggerError: haptics.triggerError }))
...
describe('the quick-command row when the pasteboard refuses the text', () => {
  ...
  it('says it could not copy and buzzes the error', async () => {
    ...
    expect(haptics.triggerError).toHaveBeenCalledOnce()
  })
