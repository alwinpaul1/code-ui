...
export function useMobileSessionTabApplication(scope: MobileSessionTerminalListModel) {
  ...
  const applySessionTabs = useCallback(
    (result: SessionTabsResult): SessionTabsApplyOutcome<MobileSessionTab> => {
      ...
      // Desktop-driven activations count as visits too, so a close after the
      // desktop switched tabs still returns to what the user last saw.
      recordSessionTabVisit(visitedSessionTabIdsRef.current, active?.id)
