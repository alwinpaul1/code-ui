...
// Ratchet gate for the mobile test typecheck. Ported from Orca #21298 — this fork generated its own
// baseline against its own test suite rather than copying upstream's numbers or file list.
//
...
// existed. This check freezes the current failing set and fails when a test file that typechecks
// today stops doing so. The baseline may only shrink.
...
//
// Upstream Orca excludes four Node-side test files here (desktop-main-process / child-process
// imports checked against @types/node rather than RN's libs). This fork has none of those —
// `grep -rl "from '.*/src/main\|shared/child-process'" mobile/src mobile/scripts | grep '\.test\.ts'`
// returns nothing (2026-09-19) — so this map is empty and tsconfig.test.json excludes only
// node_modules. The census test below (`matches tsconfig.test.json exclude entry for entry`) keeps
