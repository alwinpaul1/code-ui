...
| 2fc84cb49 #20493 | one tail-follow owner so streaming stops jumping | **done, partial.** Upstream's symptom was two tail-followers racing: a delayed *animated* `scrollToEnd` easing toward a stale endpoint while a non-animated content-size pin yanked past it. **This fork never had that race** — its inverted FlashList has no timer-driven scroll at all, and `onContentSizeChange` is its only tail-follower, already non-animated and already gated by `mobile-chat-follow-gate.ts`. The new upstream file `use-mobile-native-chat-tail-follow.ts` is **not** vendored: it owns a non-inverted `FlatList` and pins with `scrollToEnd` / `scrollToOffset({offset: measuredHeight})`, where our tail is the constant offset 0, and it has no room for this fork's hold/long-press ownership, dock-height re-pin or scroll-bend. Taking it would have replaced three working owners with one that cannot express them. **What was ported is the third commit in the PR, the momentum fence:** a released finger is no longer treated as a settled list. `use-mobile-chat-scroll-handlers.ts` now defers the end-drag settle one animation frame; `onScrollBeginDrag`, `onMomentumScrollBegin` and `jumpToLatest` cancel that frame, and `onMomentumScrollEnd` settles on where the fling actually landed. The two inline momentum handlers moved out of the view into the hook, so the list has one scroll owner. **Also not ported:** `onLayout={pinToTail}` (an inverted list keeps its tail at the bottom of a shrinking viewport by itself, and `use-mobile-chat-dock.ts` already re-pins on the one resize that matters), and `showJumpToTail = !following && !atTail` (this fork deliberately raises the control the instant a drag starts, pinned by its own test) |
